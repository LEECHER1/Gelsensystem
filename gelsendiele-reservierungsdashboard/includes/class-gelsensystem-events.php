				...
				<section class="gelsensystem-events-inventory">
					...
					<div class="gelsensystem-events-list">
						...
						<?php foreach ( $events as $event ) : ?>
							...
							$overview_id = 'gse-admin-event-overview-' . absint( $event['id'] );
							?>
							<article class="<?php echo $event['active'] ? '' : 'is-inactive'; ?>" style="--gse-admin-accent:<?php echo esc_attr( $event['color'] ); ?>" data-gse-admin-event-card>
	...
	public static function render_homepage_popup() {
		...
		<div class="gse-event-popup" data-gse-popup data-event-id="<?php echo esc_attr( $event['id'] ); ?>" data-popup-version="<?php echo esc_attr( md5( $event['popup_start'] . '|' . $event['popup_end'] ) ); ?>" style="<?php echo esc_attr( Gelsendiele_Settings::css_variables() ); ?>--gse-accent:<?php echo esc_attr( $event['color'] ); ?>" hidden>
			...
			<section class="gse-event-popup__dialog<?php echo $event['image_id'] ? ' has-image' : ''; ?>" role="dialog" aria-modal="true" aria-labelledby="gse-popup-title-<?php echo esc_attr( $event['id'] ); ?>">
				...
				<div class="gse-event-popup__content"><span>Nächstes Event · <?php echo esc_html( self::format_event_time( $event ) ); ?></span><h2 id="gse-popup-title-<?php echo esc_attr( $event['id'] ); ?>"><?php echo esc_html( $event['title'] ); ?></h2><?php if ( $event['description'] ) : ?><div><?php echo wp_kses_post( wpautop( $event['description'] ) ); ?></div><?php endif; ?><a href="<?php echo esc_url( home_url( '/events/#gse-event-' . absint( $event['id'] ) ) ); ?>">Zum Event</a></div>
			</section>
		</div>
		<?php
	}
